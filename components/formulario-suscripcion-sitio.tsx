...
import { suscribirseNewsletterSitio } from '@/app/admin/actions'

export default function FormularioSuscripcionSitio({ sitioId, colorPrimario }: { sitioId: string; colorPrimario: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    formData.set('sitio_id', sitioId)
    const resultado = await suscribirseNewsletterSitio(formData)
    if (resultado?.error) {
      setEstado('error')
    ...
    <form onSubmit={handleSubmit} className="flex flex-col w-full md:w-auto md:min-w-[420px]">
      <div className="flex w-full">
        ...
        <button
          ...
          className="text-white font-extrabold text-xs px-6 rounded-r-lg transition disabled:opacity-60 hover:opacity-90"
        >
          {estado === 'cargando' ? 'ENVIANDO...' : 'SUSCRIBIRME'}
        </button>
      </div>
      {estado === 'error' && <span className="text-red-500 text-xs mt-2">{mensaje}</span>}
    </form>
  )
}